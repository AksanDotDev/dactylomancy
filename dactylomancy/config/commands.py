...
async def setup(bot: DactylomancyBot):
    ...
    # If the minimise flag is set then only register the command to expand the config
    if config_table['minimise']:
        _log.info('Config group minimized, added command to enable.')

        @bot.tree.add_command(
            name='config',
            description='Enable configuration options, reload the bot extensions, and resync the command tree.',
        )
        @bot.user_only()
        async def config(
            ...
            _log.debug('Enabling configuration options.')
            config_table['minimise'] = False
            await bot.reload_extensions()
            await bot.tree.sync()
            await interaction.followup.send(
                embed=bot.config_embed.get_simple_embed(
                        'Config',
                        'Enabled and resynced.\n'
                        + 'Sync to clients may take 2 hours to propogate.\n'
                        + 'Reloading Discord (`ctrl + R`) may preempt this.\n'
                    ),
                ephemeral=True,
            )
    else:
        ...
        async def share(interaction: discord.Interaction):
            _log.debug('Returing sanitised config file for sharing.')
        ...
        async def sync(interaction: discord.Interaction):
            ...
            _log.debug('Initiated user requested sync.')
            await bot.tree.sync()
            await interaction.followup.send(
                embed=bot.config_embed.get_simple_embed(
                        'Sync',
        ...
        async def core_settings(
            ...
            if prefix_str and not clear_prefix:
                if not PREFIX_REGEX.match(prefix_str):
                    ...
                    _log.debug(f'Setting core prefix to {prefix_str.lower()}.')
                    ...
                    update_message += 'Reloading Discord (`ctrl + R`) may preempt this.\n'
            elif clear_prefix:
                _log.debug('Clearing core prefix.')
        ...
        async def embed_settings(
            ...
            if duration is not None:
                _log.debug(f'Setting embed duration to {duration} seconds.')
            ...
            if colour_str:
                _log.debug(f'Attempting to parse colour string {colour_str} for config embed.')
                try:
                    ...
                    _log.debug(f'Sucessfully parsed colour string {colour_str} for config embed.')
                except ValueError:
                    update_message = f'Config embed colour unchanged, falied to parse: `{colour_str}`.\n'
                    _log.debug(f'Failed to parse colour string {colour_str} for config embed.')
            else:
                update_message = ''
        ...
        async def config(
            ...
            logging.debug('Disabling configuration options.')
            config_table['minimise'] = False
            await bot.reload_extensions()
            await bot.tree.sync()
            await interaction.followup.send(
